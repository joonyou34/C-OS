...
void *create_user_kern_stack(uint32 *ptr_user_page_directory)
{
	...
	// Allocate space for the kernel stack (including space for the guard page)
	uint32 stackSize = KERNEL_STACK_SIZE;
	uint32 *stack_start_address = (uint32 *)kmalloc(stackSize);
	if (stack_start_address == NULL)
	{
		panic("Failed to allocate kernel stack");
	}

	uint32 *guard_address = (uint32 *)ROUNDDOWN((uint32)stack_start_address, PAGE_SIZE);

	// guard page not present
	uint32 *page = NULL;
	get_page_table(ptr_user_page_directory, (uint32)guard_address, &page);

	if (page == NULL)
	{
		panic("Page table for guard page not found");
	}

	page[PTX(guard_address)] &= ~PERM_PRESENT;

	return (void *)stack_start_address;
#else
...
void initialize_uheap_dynamic_allocator(struct Env *e, uint32 daStart, uint32 daLimit)
{
	...
	e->start = e->brk = (uint32 *)daStart;
	e->limit = (uint32 *)daLimit;
