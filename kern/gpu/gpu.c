...
void initialize_gpu(struct vbe_mode_info_structure *VESA_mode_info) {
    // in case for some reason the boot version of this function is never called
    // then we make sure to still initalize the lock and mark the GPU as on
    if(!GPU_ON) {
        init_spinlock(&GPU.lock, "framesuffer");

        GPU_ON = 1;
    }
    // adding kernel base to the vesa mode info block to be able to use it in protected mode
...
void boot_initialize_gpu(struct vbe_mode_info_structure *VESA_mode_info) {
    // pagin is not on yet, so we can safely add kernel base to the addresses to use them
    GPU.mode_info = (struct vbe_mode_info_structure *)(((char *)VESA_mode_info + KERNEL_BASE));
...
void kdraw_pixel(uint32 row, uint32 col, color_t color_rgba){
...
void kdraw_pixel_hex(uint32 row, uint32 col, uint32 color_hex) {
    kdraw_pixel(row, col, HEX_TO_COLOR(color_hex));
}
...
void kdraw_rectangle_filled(uint32 top_left_row, uint32 top_left_col, uint32 bot_right_row, uint32 bot_right_col, color_t color_rgba) {
    for(uint32 row = top_left_row; row < bot_right_row; row++) {
        for(uint32 col = top_left_col; col < bot_right_col; col++) {
            kdraw_pixel(row, col, color_rgba);
        }
    }
}
...
void kdraw_rectangle_filled_hex(uint32 top_left_row, uint32 top_left_col, uint32 bot_right_row, uint32 bot_right_col, uint32 color_hex) {
    kdraw_rectangle_filled(top_left_row, top_left_col, bot_right_row, bot_right_col, HEX_TO_COLOR(color_hex));
}

void kdraw_square_filled(uint32 top_left_row, uint32 top_left_col, uint32 size, color_t color_rgba) {
    kdraw_rectangle_filled(top_left_row, top_left_col, top_left_row+size, top_left_col+size, color_rgba);
}

void kdraw_square_filled_hex(uint32 top_left_row, uint32 top_left_col, uint32 size, uint32 color_hex) {
    kdraw_square_filled(top_left_row, top_left_col, size, HEX_TO_COLOR(color_hex));
}

void kdraw_char(uint32 row, uint32 col, char character, uint32 scale, color_t text_color_rgba, color_t bg_color_rgba) {
    uint8 character_idx = character;
    for (uint32 row_off = 0; row_off < 8; row_off++) {
		uint8 row_data = font_data[character_idx][row_off];
		
		for (uint32 col_off = 0; col_off < 8; col_off++) {
            if (row_data & (1 << col_off))
                kdraw_square_filled(row+row_off*scale, col+col_off*scale, scale, text_color_rgba);

            else if(bg_color_rgba.alpha)
                kdraw_square_filled(row+row_off*scale, col+col_off*scale, scale, bg_color_rgba);
        }
    }
}

void kdraw_char_hex(uint32 row, uint32 col, char character, uint32 scale, uint32 text_color_hex, uint32 bg_color_hex) {
    kdraw_char(row, col, character, scale, HEX_TO_COLOR(text_color_hex), HEX_TO_COLOR(bg_color_hex));
}
